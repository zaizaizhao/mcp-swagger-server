...
export const useTestingStore = defineStore('testing', () => {
  ...
  // 参数验证
  const validateParameters = (tool: MCPTool, parameters: Record<string, any>): string[] => {
    const errors: string[] = []
    ...
    for (const requiredParam of required) {
      if (!(requiredParam in parameters) || parameters[requiredParam] === undefined || parameters[requiredParam] === '') {
    ...
    for (const [paramName, paramValue] of Object.entries(parameters)) {
      ...
      const typeError = validateParameterType(paramName, paramValue, paramSchema)
      if (typeError) {
  ...
  // 验证参数类型
  const validateParameterType = (name: string, value: any, schema: any): string | null => {
  ...
  // 生成模拟响应
  const generateMockResponse = (tool: MCPTool, parameters: Record<string, any>) => {
    const responses = {
      'get': {
