...
export const useLogStore = defineStore('log', () => {
  ...
  const filteredLogs = computed(() => {
    return logs.value.filter(log => {
      ...
      // 来源过滤
      if (filters.value.sources.length > 0 && !filters.value.sources.includes(log.source)) {
        return false
      }
      ...
      if (filters.value.tags.length > 0) {
        if (!log.tags || !filters.value.tags.some(tag => log.tags!.includes(tag))) {
          return false
        }
      }
  ...
  const logStats = computed((): LogStats => {
    ...
    const errorCount = stats.byLevel[LogLevel.ERROR] + stats.byLevel[LogLevel.FATAL]
  ...
  const exportLogs = async (options: LogExportOptions): Promise<Blob> => {
    const logsToExport = filteredLogs.value.filter(log => {
      return log.timestamp >= options.timeRange.start && 
      ...
      case 'csv':
        ...
        content = [
          ...
          ...logsToExport.map(log => [
            ...
            ...(options.includeContext ? [JSON.stringify(log.context || {})] : []),
            ...(options.includeStack ? [`"${(log.stack || '').replace(/"/g, '""')}"`] : [])
          ].join(','))
        ].join('\n')
        break
