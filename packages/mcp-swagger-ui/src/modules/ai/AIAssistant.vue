  ...
  <div class="ai-assistant">
    ...
    <div class="header-section">
      ...
      <div class="header-actions">
        ...
        <el-button @click="importConfig" :icon="Upload">
          导入配置
        </el-button>
      </div>
    </div>
      ...
      <el-card shadow="never">
        <template #header>
          <div class="card-header">
            <span>选择AI助手类型</span>
            <el-tag type="info" size="small">{{ selectedType ? `已选择：${getAssistantName(selectedType)}` : '请选择助手类型' }}</el-tag>
          </div>
        </template>
        ...
        <div class="assistant-grid">
          <div
            ...
            :key="assistant.type"
            :class="['assistant-card', { active: selectedType === assistant.type }]"
          ...
          <el-card shadow="never">
            <template #header>
              <div class="card-header">
                <span>配置参数</span>
                <el-button-group>
                  <el-button size="small" @click="useTemplate" :icon="Promotion">
                    使用模板
                  </el-button>
            ...
            >
              ...
              <!-- 动态配置字段 -->
              <template v-for="field in getConfigFields(selectedType)" :key="field.key">
            ...
            <template #header>
              <div class="card-header">
                ...
                <el-button-group>
                  ...
                  <el-button size="small" @click="copyConfig" :icon="DocumentCopy">
                    复制
                  </el-button>
                  <el-button size="small" @click="exportConfig" :icon="Download">
                    导出
                  </el-button>
                </el-button-group>
              </div>
    ...
    <div v-if="selectedType" class="history-section">
      <el-card shadow="never">
        <template #header>
          <div class="card-header">
            <span>配置历史</span>
            <el-button size="small" @click="clearHistory" :icon="Delete" type="danger">
              清空历史
            </el-button>
          </div>
        </template>
          ...
          >
            <div class="history-info">
              ...
              <div class="history-meta">
                <el-tag size="small" type="info">{{ item.assistantType }}</el-tag>
            ...
            <div class="history-actions">
              ...
              <el-button size="small" @click="useHistory(item)" :icon="CopyDocument">
                使用
              </el-button>
              <el-button size="small" @click="deleteHistory(index)" :icon="Delete" type="danger">
                删除
              </el-button>
            </div>
          </div>
    ...
    >
      <el-form :model="newConfigForm" label-width="100px">
        ...
        <el-form-item label="AI助手类型">
          <el-select v-model="newConfigForm.type" placeholder="选择AI助手类型" style="width: 100%">
            <el-option
              v-for="assistant in assistantTypes"
              :key="assistant.type"
        ...
        <el-table :data="configTemplates" style="width: 100%">
          ...
          <el-table-column label="操作" width="200">
            <template #default="{ row, $index }">
              <el-button size="small" @click="useConfigTemplate(row)" :icon="Tools">
                使用
              </el-button>
              <el-button size="small" @click="editTemplate(row)" :icon="Edit">
                编辑
              </el-button>
              <el-button size="small" @click="deleteTemplate($index)" :icon="Delete" type="danger">
                删除
              </el-button>
            </template>
          </el-table-column>
...
import {
  Service, Plus, Folder, Upload, Setting, Promotion, RefreshRight,
  Edit, Link, Check, DocumentCopy, Download, Delete, View,
  CopyDocument, Tools
...
const generateConfig = () => {
  ...
  const config = {
    type: 'mcp-server',
    name: configForm.name || `${getAssistantName.value(selectedType.value)} Config`,
    assistant: {
      type: selectedType.value,
      ...Object.fromEntries(
        Object.entries(configForm).filter(([key]) => 
          !['name', 'serverUrl', 'timeout', 'retries', 'enableLogging', 'tags'].includes(key)
        )
      )
    },
    server: {
...
// 保存数据到localStorage
watch(configHistory, () => {
  localStorage.setItem('ai-assistant-config-history', JSON.stringify(configHistory.value))
}, { deep: true })

watch(configTemplates, () => {
  localStorage.setItem('ai-assistant-config-templates', JSON.stringify(configTemplates.value))
}, { deep: true })
</script>
