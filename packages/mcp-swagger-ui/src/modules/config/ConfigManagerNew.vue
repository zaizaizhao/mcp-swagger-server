<template>
  <div class="config-manager">
              ...
              >
                <div class="config-header">
                  <div class="config-title">
                    <el-icon class="config-icon" :class="getConfigIcon(config.type)">
                      <Setting />
                    </el-icon>
                    <span>{{ getConfigLabel(config.type) }}</span>
                  </div>
                  <el-tag 
                    :type="getConfigStatus(config)"
                    size="small"
                  >
                    {{ config.count }} 项
                  </el-tag>
                ...
                <div class="config-actions">
                  <el-button size="small" @click="previewConfig(config.type)" :icon="View">
                    预览
                  </el-button>
                  <el-button size="small" @click="exportSingleConfig(config.type)" :icon="Download">
                    导出
                  </el-button>
                </div>
              </div>
    ...
    >
      ...
      <div v-if="importStep === 2" class="import-step">
          ...
          <div class="conflicts-list">
            ...
            >
              <div class="conflict-header">
                ...
                <el-tag size="small">{{ getConfigLabel(conflict.type) }}</el-tag>
              </div>
        ...
        <div v-else class="no-conflicts">
          <el-result icon="success" title="没有发现冲突" sub-title="可以直接导入配置">
          </el-result>
        </div>
      </div>
    ...
    <!-- 配置预览对话框 -->
    <el-dialog v-model="previewDialogVisible" :title="`${getConfigLabel(previewConfigType)} 预览`" width="800px">
      <div class="preview-content">
        <div class="preview-toolbar">
          ...
          <el-button size="small" :icon="CopyDocument" @click="copyPreviewContent">
            复制
          </el-button>
        </div>
        ...
        <div class="basic-info">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <span class="value" :class="selectedHistoryItem.status === 'success' ? 'success' : 'error'">
...
import { useConfigStore, type ConfigExportOptions, type ConfigValidationResult, type ConfigConflict } from '@/stores/config'
...
const generateMockConfig = (type: string): any => {
  const mockConfigs: Record<string, any> = {
    servers: [
      { id: '1', name: 'API Server', host: 'localhost', port: 3000, protocol: 'http' },
      { id: '2', name: 'Database Server', host: 'db.example.com', port: 5432, protocol: 'tcp' }
    ],
    auth: [
      { id: '1', type: 'bearer', name: 'API Token', description: 'Bearer token authentication' },
      { id: '2', type: 'basic', name: 'Basic Auth', description: 'Username/password authentication' }
    ],
    openapi: [
      { id: '1', name: 'User API', version: '1.0.0', path: '/api/v1/users' },
      { id: '2', name: 'Product API', version: '2.0.0', path: '/api/v2/products' }
    ],
    testcases: [
...
onMounted(() => {
  // 初始化一些示例操作历史
  addOperationHistory('export', '导出服务器配置', '成功导出3个服务器配置', true, { type: 'servers', count: 3 })
  addOperationHistory('import', '导入认证配置', '成功导入2个认证配置', true, { type: 'auth', count: 2 })
})
</script>
