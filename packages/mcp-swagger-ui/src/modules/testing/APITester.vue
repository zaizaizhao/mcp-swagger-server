              ...
              <div class="history-list">
                ...
                >
                  <div class="history-info">
                    <div class="history-tool">{{ getToolName(historyItem.toolId) }}</div>
                    <div class="history-time">{{ formatDateTime(historyItem.timestamp) }}</div>
                  </div>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="createTestCaseDialogVisible = false">取消</el-button>
      ...
      <div v-if="selectedHistoryItem" class="history-details">
        <div class="detail-section">
          ...
          <div class="detail-content">
            <p><strong>工具:</strong> {{ getToolName(selectedHistoryItem.toolId) }}</p>
            <p><strong>时间:</strong> {{ formatDateTime(selectedHistoryItem.timestamp) }}</p>
            <p><strong>状态:</strong> 
              <el-tag :type="selectedHistoryItem.result.success ? 'success' : 'danger'">
                {{ selectedHistoryItem.result.success ? '成功' : '失败' }}
              </el-tag>
            </p>
            <p><strong>执行时间:</strong> {{ selectedHistoryItem.result.executionTime }}ms</p>
          </div>
        </div>
        ...
        <div class="detail-section">
          ...
          <el-input
            type="textarea"
            :value="selectedHistoryItem.result.success 
...
const filteredTools = computed(() => {
  ...
  return tools.value.filter((tool: MCPTool) => 
...
const filteredTestCases = computed(() => {
  ...
  if (selectedTag.value) {
    filtered = filtered.filter(tc => 
      tc.tags.includes(selectedTag.value)
    )
  }
...
const executeTest = async () => {
  ...
  try {
    const result = await measureFunction('executeTest', async () => {
      return await testingStore.executeToolTest(selectedTool.value!, testParameters.value)
    })
...
const getParameterRules = (paramName: string, schema: any) => {
  ...
  if (isRequired(paramName)) {
    rules.push({ required: true, message: `${paramName} 是必需参数`, trigger: 'blur' })
  }
  
  if (schema.type === 'string' && schema.minLength) {
    rules.push({ min: schema.minLength, message: `最少 ${schema.minLength} 个字符`, trigger: 'blur' })
  }
  
  if (schema.type === 'string' && schema.maxLength) {
    rules.push({ max: schema.maxLength, message: `最多 ${schema.maxLength} 个字符`, trigger: 'blur' })
  }
