          ...
          <el-col :span="18">
            <el-row :gutter="16">
              ...
              <el-col :span="6">
                <MetricCard
                  title="网络I/O"
                  :value="((currentMetrics?.network?.bytesIn || 0) + (currentMetrics?.network?.bytesOut || 0)) / 1024 / 1024"
                  unit="MB/s"
                  :data="networkHistory"
                  :threshold="100"
                  type="network"
...
const systemStatusType = computed(() => {
  const critical = alerts.value.filter(a => a.level === 'critical' && !a.acknowledged).length
  const warning = alerts.value.filter(a => a.level === 'warning' && !a.acknowledged).length
...
const metricsTableData = computed(() => {
  return monitoringStore.metrics.slice(-50).reverse().map((m: any) => ({
    timestamp: m.timestamp,
    cpu: m.cpu.usage,
...
const systemStatusData = computed(() => ({
  status: systemStatusType.value === 'success' ? 'healthy' as const : 
          systemStatusType.value === 'warning' ? 'warning' as const : 'critical' as const,
  services: [
    ...
    { name: 'WebSocket', status: monitoringStore.isConnected ? 'online' : 'offline', icon: 'Connection' },
...
const unwatchAlerts = monitoringStore.$subscribe((mutation, state) => {
  ...
  if (events.some((e: any) => e.key === 'alerts' && e.type === 'add')) {
    const newAlerts = state.alerts.filter((alert: any) => 
      !alert.acknowledged && 
