        ...
        >
          <template v-for="route in menuRoutes" :key="route.name">
            ...
            >
              ...
              <template #title>
                <span>{{ $t(`menu.${String(route.name)}`) || route.meta?.title }}</span>
                <el-tooltip 
        ...
        <div class="header-left">
          ...
          <el-breadcrumb separator="/" class="breadcrumb">
            ...
            <el-breadcrumb-item v-if="currentRoute?.meta?.title">
              {{ $t(`menu.${String(currentRoute.name)}`) || currentRoute.meta.title }}
            </el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        ...
        <div class="header-right">
          ...
          >
            <el-button 
              ...
              :title="systemStatus.isHealthy ? $t('common.healthy') : $t('common.unhealthy')"
            />
          </el-badge>
...
const getIconComponent = (iconName?: string | unknown) => {
  if (!iconName || typeof iconName !== 'string' || !iconComponents[iconName as keyof typeof iconComponents]) {
    return Monitor
  }
...
const toggleTheme = () => {
  try {
    ...
    ElMessage.success(t('theme.switched', { theme: themeStore.isDark ? t('theme.dark') : t('theme.light') }))
  } catch (error: any) {
    globalErrorHandler?.captureError(error, {
...
const handleLanguageChange = (locale: Locale) => {
  try {
    ...
    ElMessage.success(t('language.switched', { language: language?.label || locale }))
  } catch (error: any) {
    globalErrorHandler?.captureError(error, {
...
const showSystemStatus = () => {
  try {
    if (systemStatus.value.isHealthy) {
      ElMessage.success(t('dashboard.systemStatus') + ': ' + t('common.healthy'))
    } else {
      ElMessage.warning(t('dashboard.systemStatus') + ': ' + t('common.unhealthy'))
    }
  } catch (error: any) {
    globalErrorHandler?.captureError(error, {
