...
export const documentsApi = {
  ...
  async getDocuments(): Promise<Document[]> {
    try {
      const response: AxiosResponse<PaginatedResponse<Document>> = await apiClient.get('/documents')
      // 从分页响应中提取documents数组
  ...
  async createDocument(data: CreateDocumentDto): Promise<Document> {
    try {
      const response: AxiosResponse<Document> = await apiClient.post('/documents', data)
      return response.data
    } catch (error) {
  ...
  async checkAuth(): Promise<boolean> {
    try {
      ...
      console.log('Auth check successful, documents loaded:', response.data?.length || 0)
      return true
    } catch (error: any) {
