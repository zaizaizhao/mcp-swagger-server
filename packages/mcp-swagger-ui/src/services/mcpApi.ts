...
import type { ApiResponse, MCPServer, MCPTool, InputSource, ConvertConfig } from '@/types'
...
class MCPApiService {
  ...
  async executeServerTool(serverId: string, toolId: string, parameters: any): Promise<ApiResponse<any>> {
    try {
      const response = await this.api.post(`/servers/${serverId}/tools/${toolId}/execute`, {
        parameters
      })
      return response.data
    } catch (error) {
