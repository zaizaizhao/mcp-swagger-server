...
export const validateParametersAgainstSchema = (
  ...
  if (schema.properties) {
    for (const [key, value] of Object.entries(parameters)) {
      ...
      if (propertySchema) {
        const propertyErrors = validateValueAgainstProperty(value, propertySchema, key)
  ...
  if (schema.type === 'array' && Array.isArray(value)) {
    if (schema.items) {
      value.forEach((item, index) => {
        const itemErrors = validateValueAgainstProperty(item, schema.items!, `${path}[${index}]`)
  ...
  if (schema.type === 'object' && typeof value === 'object' && value !== null) {
    if (schema.properties) {
      for (const [key, subValue] of Object.entries(value)) {
        ...
        if (subSchema) {
          const subErrors = validateValueAgainstProperty(subValue, subSchema, `${path}.${key}`)
 ...
 * 验证字符串格式
 */
const validateStringFormat = (value: string, format: string, path: string): ValidationError[] => {
  const errors: ValidationError[] = []
 ...
 * 为属性生成值建议
 */
const generateValueSuggestions = (schema: PropertySchema, currentValue?: any): any[] => {
...
const generateStringSuggestions = (format?: string): string[] => {
  switch (format) {
    ...
    case 'uri':
    case 'url':
      return ['https://api.example.com', 'https://www.example.com', 'http://localhost:3000']
    case 'date':
      const today = new Date().toISOString().split('T')[0]
      return [today, '2024-01-01', '2024-12-31']
    case 'date-time':
      return [new Date().toISOString(), '2024-01-01T00:00:00Z', '2024-12-31T23:59:59Z']
    case 'uuid':
 ...
 * 检查参数是否为必需
 */
export const isRequiredParameter = (parameterName: string, schema: ParameterSchema): boolean => {
 ...
 * 获取参数的描述信息
 */
export const getParameterDescription = (parameterName: string, schema: ParameterSchema): string => {
 ...
 * 获取参数的类型信息
 */
export const getParameterType = (parameterName: string, schema: ParameterSchema): string => {
  const property = schema.properties?.[parameterName]
 ...
 * 检查参数是否有枚举值
 */
export const hasEnumValues = (parameterName: string, schema: ParameterSchema): boolean => {
  const property = schema.properties?.[parameterName]
 ...
 * 获取参数的枚举值
 */
export const getEnumValues = (parameterName: string, schema: ParameterSchema): any[] => {
  const property = schema.properties?.[parameterName]
