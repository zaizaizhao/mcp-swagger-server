...
class GlobalErrorHandler {
  ...
  private showErrorNotification(error: AppError) {
    ...
    const isCritical = error.code === 'VUE_ERROR' || error.code === 'JAVASCRIPT_ERROR'
  ...
  getErrorStats() {
    ...
    const errors24h = this.errorHistory.filter(error => error.timestamp >= last24h)
    const errorsLastHour = this.errorHistory.filter(error => error.timestamp >= lastHour)
...
class GlobalNotificationManager {
  ...
  private processQueue() {
    if (this.notificationQueue.length > 0 && this.activeScount < this.maxConcurrent) {
