 ...
 * 解析OpenAPI内容
 */
export const parseOpenAPI = (content: string): { data: any; error?: string } => {
  try {
    // 尝试解析为JSON
 ...
 * 从OpenAPI规范提取API路径
 */
export const extractApiPaths = (content: string): Array<{
  method: string
  path: string
  summary?: string
  description?: string
 ...
 * 计算OpenAPI规范统计信息
 */
export const calculateOpenAPIStats = (content: string): {
  pathCount: number
  toolCount: number
 ...
 * 格式化OpenAPI内容
 */
export const formatOpenAPI = (content: string, format: 'json' | 'yaml' = 'yaml'): string => {
  try {
    const { data, error } = parseOpenAPI(content)
